...
//****************************************************************************
// utility
dReal dxWorld::global_cfm = REAL(2.7363636e-10);
dReal dxWorld::global_erp=REAL(0.2);
dVector3 dxWorld::gravity={REAL(0.),REAL(-1.),REAL(0.)};
dxAutoDisable dxWorld::adis = {REAL(0.001)*REAL(0.001), REAL(0.001)*REAL(0.001), 10, 0};

int dxWorld::body_flags=0;			// auto-disable flag for new bodies
...
{
    dAASSERT(b);
    return 0;//b->adis.average_samples;
}
...
{
    //dAASSERT(w);
	w->body_flags = (do_auto_disable != 0);
}

...
void dxConvex::computeAABB()
{
    ...
    dVector3 point;
    dMultiply0_331 (point,R,points);
    aabb[0] = point[0]+pos[0];
    aabb[1] = point[0]+pos[0];
    aabb[2] = point[1]+pos[1];
    aabb[3] = point[1]+pos[1];
    aabb[4] = point[2]+pos[2];
    aabb[5] = point[2]+pos[2];
    for(unsigned int i=3;i<(pointcount*3);i+=3)
    {
        dMultiply0_331 (point,R,&points[i]);
        aabb[0] = dMIN(aabb[0],point[0]+pos[0]);
        aabb[1] = dMAX(aabb[1],point[0]+pos[0]);
        aabb[2] = dMIN(aabb[2],point[1]+pos[1]);
        aabb[3] = dMAX(aabb[3],point[1]+pos[1]);
        aabb[4] = dMIN(aabb[4],point[2]+pos[2]);
        aabb[5] = dMAX(aabb[5],point[2]+pos[2]);
    }
}
                             ...
                             dVector3 out)
{
	...
	dMultiply1_331(b, convex->R,
        &convex->points[(polygon[pointcount-1]*3)]);
    b[0]=convex->pos[0]+b[0];
    b[1]=convex->pos[1]+b[1];
    b[2]=convex->pos[2]+b[2];
    ...
    for(unsigned i=0; i != pointcount; ++i)
    {
        ...
        dMultiply0_331 (b,convex->R,&convex->points[(polygon[i]*3)]);
        b[0]=convex->pos[0]+b[0];
        b[1]=convex->pos[1]+b[1];
        b[2]=convex->pos[2]+b[2];
...
#define BOTH_SIGNS	(LTEQ_ZERO | GTEQ_ZERO)
    ...
    {
        dMultiply0_331 (v2,Convex->R,&Convex->points[(i*3)]);
        dVector3Add(Convex->pos, v2, v2);
                          ...
                          dContactGeom *contact, int skip)
{
    ...
    */
    ...
    offsetpos[0]=Sphere->pos[0]-Convex->pos[0];
    offsetpos[1]=Sphere->pos[1]-Convex->pos[1];
    offsetpos[2]=Sphere->pos[2]-Convex->pos[2];
    for(unsigned int i=0;i<Convex->planecount;++i)
    {
        // apply rotation to the plane
        dMultiply0_331(plane,Convex->R,&Convex->planes[(i*4)]);
        ...
        if(dist>0)
        {
            ...
            if(dist<Sphere->radius)
            {
                ...
                // the plane
                if(IsPointInPolygon(Sphere->pos,pPoly,plane,Convex,out))
                {
                    ...
                    contact->pos[0] = Sphere->pos[0]+
                        (-contact->normal[0]*Sphere->radius);
                    contact->pos[1] = Sphere->pos[1]+
                        (-contact->normal[1]*Sphere->radius);
                    contact->pos[2] = Sphere->pos[2]+
                ...
                else
                {
                    ...
                    temp[0] = (Sphere->pos[0]-out[0]);
                    temp[1] = (Sphere->pos[1]-out[1]);
                    temp[2] = (Sphere->pos[2]-out[2]);
...
inline void ComputeInterval(dxConvex& cvx,dVector4 axis,dReal& min,dReal& max)
{
    ...
    dMultiply0_331(point,cvx.R,cvx.points);
    ...
    {
        dMultiply0_331(point,cvx.R,cvx.points+(i*3));
        point[0]+=cvx.pos[0];
        point[1]+=cvx.pos[1];
        point[2]+=cvx.pos[2];
                           ...
                           dContactGeom *contact, int skip)
{
    ...
    {
        // Rotate
        dMultiply0_331(e1,cvx1.R,cvx1.points+(cvx1.edges[i].first*3));
        // translate
        e1[0]+=cvx1.pos[0];
        e1[1]+=cvx1.pos[1];
        e1[2]+=cvx1.pos[2];
        // Rotate
        dMultiply0_331(e2,cvx1.R,cvx1.points+(cvx1.edges[i].second*3));
        // translate
        e2[0]+=cvx1.pos[0];
        e2[1]+=cvx1.pos[1];
        e2[2]+=cvx1.pos[2];
        ...
        {
            // Rotate
            dMultiply0_331(plane,cvx2.R,cvx2.planes+(j*4));
            dNormalize3(plane);
            // Translate
            plane[3]=
                (cvx2.planes[(j*4)+3])+
            ...
            if(IntersectSegmentPlane(e1,e2,plane,t,target->pos))
            {
                if(IsPointInPolygon(target->pos,pPoly,plane,&cvx2,q))
                {
                    ...
                    for(size_t k=0;k<cvx2.planecount;++k)
                    {
                        ...
                        // Rotate
                        dMultiply0_331(depthplane,cvx2.R,cvx2.planes+(k*4));
                                ...
                                ConvexConvexSATOutput& ccso)
{
    ...
    for(unsigned int i=0;i<cvx1.planecount;++i)
    {
        // -- Apply Transforms --
        // Rotate
        dMultiply0_331(plane,cvx1.R,cvx1.planes+(i*4));
        dNormalize3(plane);
        // Translate
        plane[3]=
            (cvx1.planes[(i*4)+3])+
                                ...
                                ConvexConvexSATOutput& ccso)
{
    ...
    for(unsigned int i = 0;i<cvx1.edgecount;++i)
    {
        ...
        dMultiply0_331(e1a,cvx1.R,cvx1.points+(cvx1.edges[i].first*3));
        dMultiply0_331(e1b,cvx1.R,cvx1.points+(cvx1.edges[i].second*3));
        e1[0]=e1b[0]-e1a[0];
        e1[1]=e1b[1]-e1a[1];
        e1[2]=e1b[2]-e1a[2];
        for(unsigned int j = 0;j<cvx2.edgecount;++j)
        {
            ...
            // we only need to apply rotation here
            dMultiply0_331 (e2a,cvx2.R,cvx2.points+(cvx2.edges[j].first*3));
            dMultiply0_331 (e2b,cvx2.R,cvx2.points+(cvx2.edges[j].second*3));
            e2[0]=e2b[0]-e2a[0];
            e2[1]=e2b[1]-e2a[1];
            e2[2]=e2b[2]-e2a[2];
            ...
            if (((dFabs(depth)+dEpsilon)<dFabs(ccso.min_depth)))
            {
                ...
                ccso.e1a[0]+=cvx1.pos[0];
                ccso.e1a[1]+=cvx1.pos[1];
                ccso.e1a[2]+=cvx1.pos[2];
                ccso.e1b[0]+=cvx1.pos[0];
                ccso.e1b[1]+=cvx1.pos[1];
                ccso.e1b[2]+=cvx1.pos[2];
                dVector3Copy(e2a,ccso.e2a);
                dVector3Copy(e2b,ccso.e2b);
                ccso.e2a[0]+=cvx2.pos[0];
                ccso.e2a[1]+=cvx2.pos[1];
                ccso.e2a[2]+=cvx2.pos[2];
                ccso.e2b[0]+=cvx2.pos[0];
                ccso.e2b[1]+=cvx2.pos[1];
                ccso.e2b[2]+=cvx2.pos[2];
            }
        }
    }
    return true;
                           ...
                           dContactGeom *contact, int skip)
{
    ...
    ccso.dist[0] = cvx2.pos[0]-cvx1.pos[0];
    ccso.dist[1] = cvx2.pos[1]-cvx1.pos[1];
    ccso.dist[2] = cvx2.pos[2]-cvx1.pos[2];
    int maxc = flags & NUMC_MASK;
    dIASSERT(maxc != 0);
    ...
    if(!CheckSATConvexFaces(cvx1,cvx2,ccso))
    {
        ...
        if(ccso.depth_type==1) // face-face
        {
            ...
            // Rotate
            dMultiply0_331(rplane,cvx1.R,cvx1.planes+(reference_side*4));
            dNormalize3(rplane);
            // Translate
            rplane[3]=
            ...
            dMultiply0_331(i2,cvx2.R,&cvx2.points[(pIncidentPoints[0]*3)]);
            dVector3Add(i2,cvx2.pos,i2);
            // Get the same point in the reference convex space
            dVector3Copy(i2,r2);
            dVector3Subtract(r2,cvx1.pos,r2);
            dVector3Copy(r2,tmp);
            dMultiply1_331(r2,cvx1.R,tmp);
            for(unsigned int i=0;i<pIncidentPoly[0];++i)
            {
                // Move i2 to i1, r2 to r1
                dVector3Copy(i2,i1);
                dVector3Copy(r2,r1);
                dMultiply0_331(i2,cvx2.R,&cvx2.points[(pIncidentPoints[(i+1)%pIncidentPoly[0]]*3)]);
                dVector3Add(i2,cvx2.pos,i2);
                // Get the same point in the reference convex space
                dVector3Copy(i2,r2);
                dVector3Subtract(r2,cvx1.pos,r2);
                ...
                for(unsigned int j=0;j<cvx1.planecount;++j)
                {
                    ...
                    if(d1*d2<0)
                    {
                        ...
                        if(!out)
                        {
                            ...
                            dMultiply0_331(tmp,cvx1.R,p);
                            dVector3Add(tmp,cvx1.pos,p);
#endif
            ...
            /* Rotate */
            dMultiply0_331(iplane,cvx2.R,cvx2.planes+(incident_side*4));
            dNormalize3(iplane);
            /* Translate */
            iplane[3]=
            ...
            {
                dMultiply0_331(i1,cvx1.R,&cvx1.points[(pReferencePoints[i]*3)]);
                dVector3Add(cvx1.pos,i1,i1);
                ...
                dVector3Copy(i1,r1);
                dVector3Subtract(r1,cvx2.pos,r1);
                dVector3Copy(r1,tmp);
                dMultiply1_331(r1,cvx2.R,tmp);
                      ...
                      int flags, dContactGeom *contact, int skip )
{
    ...
    for ( unsigned int i = 0; i < convex->planecount; ++i )
    {
        ...
        alpha = dCalcVectorDot3( plane, ray->pos ) - plane[3];
    ...
    for ( unsigned int i = 0; i < convex->planecount; ++i )
    {
        ...
        alpha = nsign * ( dCalcVectorDot3( plane, ray->pos ) - plane[3] );

        // Compute [ plane-normal DOT ray-normal ], (/flip)
        beta = dCalcVectorDot3_13( plane, ray->R+2 ) * nsign;
